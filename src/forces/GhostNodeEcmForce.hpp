/*
 * GhostNodeEcmForce.hpp
 *
 * Force class for off-lattice ghost node ECM interactions.
 * Handles both cell–ghost node and ghost–ghost spring forces,
 * as well as ECM degradation, fibre remodeling, and node removal.
 *
 * Inspired by Carrasco-Mantis et al. (2024) doi:10.1016/j.compbiomed.2024.109559
 *
 * Physics:
 *   Cell–ECM: F_ij = k_cg × ρ_j × (d_ij − s₀) × A_ij × d̂_ij
 *   ECM–ECM: F_ij = k_gg × (d_ij − s₀_gg) × A_ij × d̂_ij
 *
 *   A_ij = anisotropy factor:
 *     (1 - a + a × |cos(angle between displacement and fibre)|)
 *     Modulates resistance depending on fibre orientation:
 *       - Along fibres: reduced stiffness (easier to separate)
 *       - Across fibres: full stiffness (harder to separate)
 */
#ifndef GHOSTNODEECMFORCE_HPP_
#define GHOSTNODEECMFORCE_HPP_
...
class GhostNodeEcmForce : public AbstractForce<DIM>
{
    friend class boost::serialization::access;

private:

    /** Shared pointer to the ghost node ECM field */
    ...
    /** Last processed timestep (to guard against multiple force evaluations per step) */
...
public:
    ...
    virtual ~GhostNodeEcmForce() {}

    /**
     * Apply cell–ghost node spring forces and manage ECM dynamics.
     *
     * This method:
     *   1. Computes cell–ghost node linear spring forces with anisotropic modulation
     *   2. Applies forces to cell nodes
     *   3. Degrades ghost nodes near cells (MMP secretion)
     *   4. Applies cell traction to remodel fibre orientation
     *   5. Updates ghost node positions (ghost–ghost viscoelastic springs)
     *   6. Periodically removes depleted ghost nodes
     */
    void AddForceContribution(AbstractCellPopulation<DIM>& rCellPopulation)
    {
        ScopedTimer _prof("GhostNodeECM");

        if (!mpGhostField)
        {
            EXCEPTION("Ghost node ECM field not set! Call SetGhostField() before simulation.");
        }

        // Get simulation timestep and detect repeated force evaluations
        ...
        // Detect vertex-based population (for force distribution across element nodes)
        ...
        // Temporary storage for nearby ghost node queries
             ...
             cell_iter != rCellPopulation.End(); ++cell_iter)
        {
            ...
            {
                const GhostNode<DIM>& gn = mpGhostField->GetNode(gn_idx);
                ...
                // Anisotropic modulation by fibre direction
                double aniso_factor = 1.0;
                if (gn.anisotropy > 0.0)
                {
                    double cos_angle = std::abs(inner_prod(unit_disp, gn.fibre_direction));
                    double a = 0.5;  // anisotropy strength
                    aniso_factor = 1.0 - a * gn.anisotropy * cos_angle;
                    // ALTERED: from {1.0 - a * gn.anisotropy * (1.0 - cos_angle)} which would reduce stiffness along fibres
                }

                // Chaste spring regime (matches GeneralisedLinearSpringForce)
                // Compression (d < s₀): log repulsion (density-independent hard-core
                //   + density-dependent component)
                // Extension   (d > s₀): exponential decay attraction (density-weighted)
                ...
                if (mCellGhostRestLength > 1e-10)
                {
                    ...
                    {
                        double density_mult = gn.density;
                        if (density_mult < 0.1)
                        {
                            density_mult = 0.1;  // prevent singularity and ensure some repulsion even for degraded nodes
                        }
                        double log_term = mCellGhostRestLength * density_mult
                                          * std::log(1.0 + overlap / mCellGhostRestLength);
                        // Hard-core repulsion: density-INDEPENDENT so even degraded
                        // ghost nodes act as solid obstacles cells cannot pass through
                ...
                {
                    // Rest length = 0: pure attraction with exponential decay
                    double alpha = 5.0;
                    force_mag = mCellGhostStiffness * gn.density * overlap
                ...
                // Newton's 3rd law: apply equal-and-opposite force to the ghost node
                // so that expanding cells push ghost nodes outward
                c_vector<double, DIM> reaction = -spring_force;
                mpGhostField->AddForceToGhostNode(gn_idx, reaction);

                if (gn.density > max_density) max_density = gn.density;
            }

            // Apply force: distribute across element vertices for vertex populations
            if (p_monolayer_pop)
            {
                // 3D monolayer vertex (OrganoidChaste): MonolayerVertexElement
            ...
            {
                // 2D Chaste VertexBasedCellPopulation
            ...
            // Reuse the nearby_ghosts list from the force computation above
        ...
        // Update ghost node positions under ECM–ECM spring forces
        // (only once per timestep to avoid double-updating with adaptive methods)
        if (new_step)
        {
        ...
        if (new_step)
        {
            ...
            if (mStepCounter >= mRemovalCheckInterval)
            {
                ...
                if (removed > 0)
                {
                    std::cout << "  GhostNodeECM: removed " << removed
    ...
    void SetGhostField(boost::shared_ptr<GhostNodeEcmField<DIM>> pField)
    {
        mpGhostField = pField;
    }

    boost::shared_ptr<GhostNodeEcmField<DIM>> GetGhostField() const { return mpGhostField; }
...
EXPORT_TEMPLATE_CLASS_SAME_DIMS(GhostNodeEcmForce)

#endif // GHOSTNODEECMFORCE_HPP_
