#ifndef TESTVISCOELASTICSHEAREXPERIMENT_HPP_
#define TESTVISCOELASTICSHEAREXPERIMENT_HPP_
...
/**
 * Simple shear stress-relaxation test on a pure viscoelastic ghost node
 * network (no cells).
 *
 * Validates the Cauchy-Born unit conversion: G = E0 * kF / Δx_phys
 * (Ostoja-Starzewski, Appl. Mech. Rev. 55(1), 2002)
 *
 * Parameters can be set via environment variables for sweep scripts:
 ...
 *   SHEAR_OUTPUT_DIR — output subdirectory name (default SimpleShear_2D)
 ...
 */
class TestViscoelasticShearExperiment : public AbstractCellBasedTestSuite
{
private:
    ...
    void EnforceShearBoundaryConditions(
        ViscoelasticGhostNodeEcmField<2>& rField,
        const std::vector<unsigned>& rBottomNodes,
        const std::vector<unsigned>& rTopNodes,
        const std::vector<double>& rBottomOriginalX,
        const std::vector<double>& rBottomOriginalY,
        const std::vector<double>& rTopOriginalX,
        const std::vector<double>& rTopOriginalY,
        double shearOffset)
    {
        for (unsigned k = 0; k < rBottomNodes.size(); k++)
        {
            unsigned idx = rBottomNodes[k];
            rField.rGetNode(idx).position[0] = rBottomOriginalX[k];
            rField.rGetNode(idx).position[1] = rBottomOriginalY[k];
        }
        for (unsigned k = 0; k < rTopNodes.size(); k++)
        {
            unsigned idx = rTopNodes[k];
            rField.rGetNode(idx).position[0] = rTopOriginalX[k] + shearOffset;
            rField.rGetNode(idx).position[1] = rTopOriginalY[k];
        }
    }

public:

    void TestSimpleShearRelaxation()
    {
        ...
        const std::string outDir = GetEnvString("SHEAR_OUTPUT_DIR", "SimpleShear_2D");
        ...
        // ── Cauchy-Born conversion: G = E0_phys / Δx ──────────────
        const double E0 = G_input_Pa * spacing_m / kF;
        const double E1 = arm_ratio * E0;

        // ── Grid geometry ─────────────────────────────────────────
        const double spacing = 1.0;
        const double x_min = 0.0, x_max = 49.0;
        const double y_min = 0.0, y_max = 9.0;
        const unsigned num_cols = static_cast<unsigned>((x_max - x_min) / spacing) + 1;
        const double width  = x_max - x_min;
        const double height = y_max - y_min;

        // ── Drag: set for fast diffusion, respecting CFL ─────────
        // CFL = (E0+E1)*N*dt/eta < 2;  N=8, dt=0.001
        // eta = (E0+E1)*8*0.001 / CFL_target, with CFL_target ~0.8
        const double dt       = 0.001;
        const double cfl_target = 0.8;
        const double eta_drag = std::max(0.01, (E0 + E1) * 8.0 * dt / cfl_target);
        ...
        // Relaxation must exceed diffusion timescale: t_diff = L^2*eta/E
        ...
        std::vector<double> bottom_orig_y(bottom_nodes.size());
        ...
        std::vector<double> top_orig_y(top_nodes.size());
        ...
        *p_csv << "time,shear_displacement,shear_strain,shear_stress,shear_stress_Pa,total_force_x_top\n";
        ...
        for (unsigned step = 0; step < n_load; step++)
        {
            ...
            EnforceShearBoundaryConditions(*p_field,
                bottom_nodes, top_nodes,
                bottom_orig_x, bottom_orig_y,
                top_orig_x, top_orig_y,
                shear_offset);
            ...
            double shear_stress    = -total_fx_top / width;
        ...
        std::cout << "\n=== Simple Shear Test Results ===\n"
                  ...
                  << "Stress ratio f/p:       " << (peak_shear_stress > 0 ? final_shear_stress/peak_shear_stress : 0.0) << "\n"
...
#endif // TESTVISCOELASTICSHEAREXPERIMENT_HPP_
