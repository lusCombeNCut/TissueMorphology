...
private:
                                   ...
                                   const std::string& outputSubDir)
    {
        ...
        std::string output_dir = outputSubDir;
...
public:
    ...
    void TestCryptBuddingStiffnessSweep()
    {
        ...
        // Output directory — flat name; HPC output folder already encodes
        // model type, stiffness, and run number
        std::stringstream subdir;
        subdir << "CryptBudding2d_NodeBased_s" << std::fixed << std::setprecision(1)
               << bm_stiffness << "_r" << run_number;
