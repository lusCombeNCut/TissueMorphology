...
#   scp -r sv22482@bp1-login.acrc.bris.ac.uk:/user/work/sv22482/sim_output/analysis_lattice_comparison_<TIMESTAMP>/plots/ ./plots/
...
if [ -z "${SLURM_JOB_ID}" ]; then
    ...
    ANALYSIS_OUTPUT="/user/work/$(whoami)/sim_output/analysis_lattice_comparison_${SWEEP_TIMESTAMP}"

    ANALYSE_JOB_ID=$(sbatch --parsable \
        --job-name="Analyse_Lattice" \
...
if [ "${SWEEP_PHASE}" = "analyse" ]; then
    ...
    ANALYSIS_OUTPUT="${ANALYSIS_OUTPUT:-/user/work/$(whoami)/sim_output/analysis_lattice_comparison_${SWEEP_TIMESTAMP}}"
    ...
    echo "============================================"
    echo "  On-Lattice vs Off-Lattice — Analysis Phase"
    ...
    SCRIPT_DIR="${SOURCE_DIR}/experiments/OnLatticeVsOffLattice"
